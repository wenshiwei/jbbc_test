...
function luru(id)
{
		window.location.href="OrderIDServlet?OrderID="+id+"&Status=0";
}


function upPageCheck(nowpage,allpage) {
	if(1<nowpage){
		window.location.href="UpDatePagesServlet?NowPage="+(nowpage-1)+"&Status="+0;
	}
	else{
	alert("当前为第一页，没有上一页！");	
	}
			...
			<div
				...
				<fieldset style="width: 100%; height: 93%; border: 1px solid black;">
					<ul>
						...
						<li><a href="OpenOrderPagServlet?Status=0"><span>订单指派
							</span></a></li>
				...
				<fieldset style="width: 100%; height: 93%; border: 1px solid black;">
					<table border="" style="width: 100%; border: 0px;">
						<tr style="width: 100%; height: 5px; border: 2px solid;"
							...
							<th style="width: 20%; height: 5px; border: 2px solid;"><span>门店指派</span></th>
						</tr>
						<c:forEach items="${facOrderList }" var="factoryOrdeDate">
							<tr align="center">
								<th style="width: 20%; height: 30px; border: 1px solid;"><span>${factoryOrdeDate.getOrderId() }</span></th>
								<th style="width: 20%; height: 30px; border: 1px solid;"><span>${factoryOrdeDate.getCustormerId() }</span></th>
								<th style="width: 20%; height: 30px; border: 1px solid;"><span>${factoryOrdeDate.getDetail() }</span></th>
								<th style="width: 20%; height: 30px; border: 1px solid;"><span>
										<input type="button" value="点击指派"
										style="width: 100%; height: 30px;"
										onclick="luru(${factoryOrdeDate.getOrderId() })" />
								</span></th>
							</tr>
						</c:forEach>
					</table>
